...
function main(argv) {
  ...
  if (process.platform === "win32") {
    // On Windows, node_modules/.bin shims are .cmd files which require a shell.
    // Join into a single command string to avoid DEP0190 deprecation warning.
    const fullCmd = [command, ...args]
      .map((a) => (a.includes(" ") ? `"${a}"` : a))
      .join(" ");
    child = spawn(fullCmd, [], { stdio: "inherit", env, shell: true });
  } else {
